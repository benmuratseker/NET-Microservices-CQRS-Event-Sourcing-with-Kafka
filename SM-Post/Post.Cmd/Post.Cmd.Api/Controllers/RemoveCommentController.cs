...
namespace Post.Cmd.Api.Controllers
{
    ...
    public class RemoveCommentController : ControllerBase
    {
        ...
        public async Task<ActionResult> RemoveCommentAsync(Guid id, RemoveCommentCommand command)
        {
            ...
            {
                _logger.Log(LogLevel.Warning, ex, "Could not retrieve aggregate, client passed an incorrect post ID targettin the aggregate!");
                return BadRequest(new BaseResponse
                {
                    Message = ex.Message
                });
            ...
            {
                const string SAFE_ERROR_MESSAGE = "Error while processing request to remove a comment to a post!";
